...
/** Adds columns that shipped after a restaurant's database already existed. */
function migrate(db) {
...
export const ORDER_TYPES = ['dine_in', 'takeout'];
 ...
 */
export const createOrder = db.transaction((restaurant, tableId, cart, note, orderType) => {
  ...
  const { lastInsertRowid: orderId } = db
    .prepare(
      `INSERT INTO orders (restaurant_id, table_id, note, subtotal, service_fee, total, order_type)
       VALUES (?, ?, ?, ?, ?, ?, ?)`
    )
    .run(restaurant.id, tableId, String(note || '').slice(0, 400), subtotal, serviceFee, total, type);
